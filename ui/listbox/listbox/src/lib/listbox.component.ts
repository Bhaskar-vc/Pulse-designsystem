...
export class VcListbox implements OnChanges, OnInit {
  ...
  /**
   * When true, marks all options as selected: forces the "All" checkbox into checked state
   * and disables all options (no mouse or keyboard interaction allowed).
   * Used internally by vc-autocomplete when collapseAllSelectionToTag is active.
   * @group Props
   */
  @Input() collapseToAllTag: boolean = false;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['collapseToAllTag']) {
      this._setSelectAllCheckboxStatus();
    }
  }

  get renderOptions(): VcSelectOption[] {
    return this.collapseToAllTag
      ? this.options.map((o) => ({ ...o, disabled: true }))
      : this.options;
  }
  ...
  isSelected(index: number): boolean {
    if (this.collapseToAllTag) return true;
  ...
  handleKeyDown(event: KeyboardEvent): void {
    if (this.collapseToAllTag) return;
  ...
  handleSelectAllChange(event: VcCheckboxChangeEvent): void {
    if (event.checked !== undefined && event.indeterminate !== undefined) {
      ...
      // Emit first so upstream handlers (e.g. autocomplete with collapseAllSelectionToTag)
      // can set their state before onSelectionChange fires below.
