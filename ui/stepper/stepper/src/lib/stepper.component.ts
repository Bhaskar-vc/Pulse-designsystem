...
export type VcHorizontalStepperVariant = 'default' | 'inline';
...
export class VcStepper extends CdkStepper implements AfterContentInit {
  @ViewChildren(VcStepHeader) override _stepHeader: QueryList<VcStepHeader> =
    undefined as unknown as QueryList<VcStepHeader>;

  @ContentChildren(VcStep, { descendants: true }) override _steps: QueryList<VcStep> =
    undefined as unknown as QueryList<VcStep>;

  @Input() linearMode: boolean = true;

  @Input() stepperPosition: string | 'top' | 'bottom' = 'top';

  @Input() freeFlow: boolean = false;

  @Input() horizontalVariant: VcHorizontalStepperVariant = 'default';
