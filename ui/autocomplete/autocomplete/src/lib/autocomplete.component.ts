...
export class VcAutocomplete implements AfterContentInit, OnDestroy {
  ...
  set selected(value: VcSelectOption | VcSelectOption[] | undefined) {
    if (this.collapseAllSelectionToTag && this.isAllSelected) {
      const isAllTag = Array.isArray(value) && value.length === 1 && value[0]?.id === this.allOption.id;
      if (!isAllTag) {
  ...
  /**
   * When true, selecting "All" collapses all individual tags into a single "All" tag
   * and disables all dropdown options until deselected. Default: false.
   * @group Props
   */
  @Input() collapseAllSelectionToTag: boolean = false;

  /**
   * Custom option object used as the "All" tag when collapseAllSelectionToTag is true.
   * Defaults to { id: '-1', text: 'All' }.
   * @group Props
   */
  @Input() allOption: VcSelectOption = { id: '-1', text: 'All' };
  ...
  handleSelectAllChange(event: VcCheckboxChangeEvent): void {
    if (this.collapseAllSelectionToTag) {
      if (event.checked) {
        this.isAllSelected = true;
        this.onSelect.emit({ selected: [this.allOption] });
      } else {
        this.isAllSelected = false;
        this.onSelect.emit({ selected: [] });
      }
      // Do not emit onSelectAllCheckboxChange — selection is fully handled via onSelect above.
      return;
    }
