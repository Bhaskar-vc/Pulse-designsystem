...
export class VTooltip implements AfterContentInit, OnDestroy {
  ...
  get tooltipClass(): string {
    return ['v-tooltip', `v-tooltip-${this.color}`, this.customClass].filter(Boolean).join(' ');
  }

  get arrowClass(): string {
    return `v-tooltip-arrow v-tooltip-arrow-${this.color} v-tooltip-arrow-${this.placement}`;
  }
